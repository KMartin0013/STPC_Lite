function varargout=grace2slept_m(Dataproduct,TH,XY_buffer,Lwindow,phi,theta,omega,J,units,forcenew)
...
%                 [default: {'CSR', 'RL06', 60, 'ocean', 'Pelt17'} which 
%                 is Release level 06 data at the Center for Space Research
%                 Another Example: {'GFZ', 'RL05', 96, 'ocean', 'Pelt17'} 
%                 which is Release level 05 data from the data center at the 
%                 GeoForschungsZentrum Potsdam for application on ocean
...
Ldata = Dataproduct{3};
land_or_ocean = Dataproduct{4}; % m Here I add coefficient land_or_ocean
GIA = Dataproduct{5}; % m Here I add GIA model
dataproductstring = [Pcenter Rlevel num2str(Ldata) land_or_ocean GIA]; % m Here I add coefficient land_or_ocean
if length(Dataproduct)>5
    phi=Dataproduct{6}(1);
    theta=Dataproduct{6}(2);
    omega=Dataproduct{6}(3);
    dataproductstring = [Pcenter Rlevel num2str(Ldata) land_or_ocean GIA ...
        'p' num2str(phi) 't' num2str(theta) 'o' num2str(omega)]; % m Here I add coefficient land_or_ocean
end
...
%     [potcoffs,thedates,thedates]=grace2plmt_m(Pcenter,Rlevel,Ldata,units,0,land_or_ocean,GIA);
    % m I replace one of the thedates as the time epoch of leakage
    [potcoffs,leakage_sp,thedates]=grace2plmt_m(Pcenter,Rlevel,Ldata,units,0,land_or_ocean,GIA);
    ...
    % expansion
    % m Here I add coefficient land_or_ocean
...
%     save(fnpl,'slepcoffs','thedates','thedates');
    % m I replace one of the thedates as the time epoch of leakage
    save(fnpl,'slepcoffs','leakage_sp','thedates');

end % End if we have a save file already

% Collect output
% varns={slepcoffs,thedates,thedates,TH,G,CC,V,N};
varns={slepcoffs,leakage_sp,thedates,TH,G,CC,V,N};
% m I replace one of the thedates as the time epoch of leakage
