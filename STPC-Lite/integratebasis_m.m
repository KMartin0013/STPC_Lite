function varargout=integratebasis_m(CC,TH,J,phi,theta,Radius)
...
% Radius  The radius of Gaussian Smooth  (unit: km) [DEFAULT: 500]
%
% OUTPUT:
%
...
defval('phi',0)
defval('theta',0)
defval('Radius',500) % default no smooth
...
elseif length(J)==2
    % Run parallel is we are able (small functions with no smooth)
    parfor h=1:J(1)
        [Int,A]=plm2avg(CC{h},XY);
        eigfunINT(h) = Int;
    end
    % Run parallel is we are able (large functions with Gaussian smooth)
    parfor h=J(1)+1:J(2)
