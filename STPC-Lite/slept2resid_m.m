function varargout=slept2resid_m(slept,thedates,fitwhat,givenerrors,specialterms,CC,TH,N,Radius)
...
if nargout >= 5 && exist('CC') && exist('TH')
    ...
    if iscell(TH)
       ...
       % m do not use spline interpolation
...
%     defval('N',50);
    ...
    % m In case additional Gaussion smooth was needed
    % m if only one N is provided, this is the same as the original procedure
    % m if two N is provided, Gaussion smooth of a given radius was applied
    % m to selective SSFs ( N(1) < N <= N(2) )
    if length(N)==1
        N_all=N;
    elseif length(N)==2
        if N(1)>N(2)
            error('Noted that N(2) should not be smaller than N(1).')
        end
        N_all=N(2);
    else
    ...
    [eigfunINT] = integratebasis_m(CC,TH,N,[],[],Radius);
