% These series of Figures are used for RSE.
% Code version: 1.
% Code Note:
%   Figure 1:
%   Main features:
...
% parrent_path2=[Code_Version 'SCS'];

% suffix_area2='06_SCSpTH';

% degree 1 buffer zone
% suffix_XY_use2='60_neg1_500';
%%%%
...
% suffix_str1=[suffix_area1 '_' suffix_XY_use1 char(suffix_N_use1(3))];
%
...
% XY1=load(XY_path1);
%
% suffix_str2=[suffix_area2 '_' suffix_XY_use2 char(suffix_N_use2(3))];
%
% % please rewrite the name of this path accordingly to the output of 'V5r1.m'
...
%% basic information
% output_path='./GMT_text_r2r3';

% lamda_range=0.3:0.1:0.9;

%% for the first region
% parrent_path=parrent_path1;
% Attach_ALL=Attach_ALL1;
% SIG_use=[0.05,0.1,0.3];

%%
for SIG=1:3

    load(fullfile(getenv('IFILES'),['Results_' parrent_path3],['Main_' Attach_ALL3 'c11.mat']));

    load(fullfile(getenv('IFILES'),['Results_' parrent_path3],['Main_' Attach_ALL3 '_TP' num2str(Turning_number) ...
    ...
    turn_V3_four=turn_V_four;turn_MSSA3_four=turn_MSSA_four;

    c11cmn3=c11cmn;
    S_range3=1:MaxNumChanges_V;N_range3=1:MaxNumChanges_MSSA;
    ...
    turn_MSSA3_four_ave=zeros(MNC,MNC);
    for ss=1:MNC
        for nn=1:MNC
            V_ss=turn_V3_four(ss,used_sta_V);

            mm_sum=0;
            for vv=1:V_ss
                mm_sum=mm_sum+turn_MSSA3_four{vv}(nn,used_sta_MSSA);
            end
            turn_MSSA3_four_ave(ss,nn)=mm_sum/V_ss;
    ...
    c11cmn3=c11cmn;
    % time wasting
    [r_example3,lon3,lat3,Plm,degres]=plm2xyz(CC{1},1,c11cmn3,60);
    [lonlon3,latlat3]=meshgrid(lon3,lat3);

    % S_range=sum(V>(1-lamda_range));
    [r_example_c11cmn3,lon_c11cmn3,lat_c11cmn3,Plm,degres]=plm2xyz(CC{1},1,c11cmn3,60);
    [lonlon_c11cmn3,latlat_c11cmn3]=meshgrid(lon_c11cmn3,lat_c11cmn3);

    lon_c11cmn3_matl=lon_c11cmn3-0.5;
    lat_c11cmn3_matl=lat_c11cmn3+0.5;
    [lonlon_c11cmn3_matl,latlat_c11cmn3_matl]=meshgrid(lon_c11cmn3_matl,lat_c11cmn3_matl);

    [in3, on3]=check_polygon_in(XY,lonlon_c11cmn3,latlat_c11cmn3);

    % area-weighted should be used to more precisely calculate the regional
    % basin-averaged variables
    for i=1:length(lat_c11cmn3)
        for j=1:length(lon_c11cmn3)
            c11cmn3_area(i,j)=areaquad(lat_c11cmn3(i)-0.5,lon_c11cmn3(j)-0.5,lat_c11cmn3(i)+0.5,lon_c11cmn3(j)+0.5);
        end
    end
    ...
    t3_ss_mean_std=[];t3_ss_mean_rms=[];
    t3_nn_mean_std=[];t3_nn_mean_rms=[];
    t3_ss_nn_std=[];t3_ss_nn_rms=[];

    s3_ss_mean_std=[];s3_ss_mean_rms=[];
    s3_nn_mean_std=[];s3_nn_mean_rms=[];
    s3_ss_nn_std=[];s3_ss_nn_rms=[];
    for disp_data=1:5
        ...
        for ss=use_ss
            ...
            for nn=use_nn
                ...
                load(fullfile(getenv('IFILES'),['Results_' parrent_path3],['Main_' Attach_ALL3 '_TP' num2str(MNC) ...
                ...
                t_ss_nn(ss,nn,:)=t_res;
                t3_ss_nn_std(disp_data,ss,nn)=std(t_res);
                t3_ss_nn_rms(disp_data,ss,nn)=rms(t_res);

                s_ss_std_mean(nn,1)=s_ss_std_mean(nn,1)+sum(s_std(in3).*c11cmn3_area(in3))/sum(c11cmn3_area(in3));
                s_nn_std_mean(ss,1)=s_nn_std_mean(ss,1)+sum(s_std(in3).*c11cmn3_area(in3))/sum(c11cmn3_area(in3));
                s_ss_rms_mean(nn,1)=s_ss_rms_mean(nn,1)+sum(s_rms(in3).*c11cmn3_area(in3))/sum(c11cmn3_area(in3));
                s_nn_rms_mean(ss,1)=s_nn_rms_mean(ss,1)+sum(s_rms(in3).*c11cmn3_area(in3))/sum(c11cmn3_area(in3));

                s3_ss_nn_std(disp_data,ss,nn)=sum(s_std(in3).*c11cmn3_area(in3))/sum(c11cmn3_area(in3));
                s3_ss_nn_rms(disp_data,ss,nn)=sum(s_rms(in3).*c11cmn3_area(in3))/sum(c11cmn3_area(in3));
            end

        end

        t3_ss_mean_std(disp_data,:)=std(t_ss_mean'/numel(use_ss));
        t3_nn_mean_std(disp_data,:)=std(t_nn_mean'/numel(use_nn));
        s3_ss_mean_std(disp_data,:)=s_ss_std_mean/numel(use_ss);
        s3_nn_mean_std(disp_data,:)=s_nn_std_mean/numel(use_nn);

        t3_ss_mean_rms(disp_data,:)=rms(t_ss_mean'/numel(use_ss));
        t3_nn_mean_rms(disp_data,:)=rms(t_nn_mean'/numel(use_nn));
        s3_ss_mean_rms(disp_data,:)=s_ss_rms_mean/numel(use_ss);
        s3_nn_mean_rms(disp_data,:)=s_nn_rms_mean/numel(use_nn);
    end
    ...
    for disp_data=1:5
        ...
        for ii=1:numel(use_nn)
            ...
            s3_ns_std(disp_data,ii)=sum(s_std(in3).*c11cmn3_area(in3))/sum(c11cmn3_area(in3));
            s3_ns_rms(disp_data,ii)=sum(s_rms(in3).*c11cmn3_area(in3))/sum(c11cmn3_area(in3));

        end

        t3_ns_std(disp_data,:)=std(t_ns');
        t3_ns_rms(disp_data,:)=rms(t_ns');
    ...
    use_ss=1:2:numel(S_range3);
    use_nn=1:2:numel(N_range3);
    ...
    for ins=1
        ...
        for ss=use_ss
            ...
            SSF=turn_V3_four(ss,used_sta_V);
            ...
            for nn=use_nn
                ...
                switch disp_data
                    case 1
                        r_res=squeeze(A5_Centers_STD_Compare(ins).fill_Grid_EWH_MSSA_uptoS_STD(SSF,:,:));fig_name=[parrent_path3 '_fill_Grid_EWH_MSSA_uptoS_STD'];
                    case 2
                        r_res=squeeze(A5_Centers_STD_Compare(ins).fill_Grid_EWH_MSSA_resid_uptoS_STD(SSF,:,:));fig_name=[parrent_path3 '_fill_Grid_EWH_MSSA_resid_uptoS_STD'];
                    case 3
                        r_res=squeeze(A5_Centers_STD_Compare(ins).fill_Grid_EWH_MSSA_bothfail_uptoS_STD(SSF,:,:));fig_name=[parrent_path3 '_fill_Grid_EWH_MSSA_bothfail_uptoS_STD'];
                    case 4
                        r_res=squeeze(A5_Centers_STD_Compare(ins).fill_Grid_EWH_MSSA_both_uptoS_STD(SSF,:,:));fig_name=[parrent_path3 '_fill_Grid_EWH_MSSA_both_uptoS_STD'];
                end

                %
                m_proj('stereographic','long',(c11cmn3(1)+c11cmn3(3))/2,'lat',(c11cmn3(4)+c11cmn3(2))/2,...
                    'rad',20,'rec','on');
                m_pcolor(lonlon_c11cmn3_matl,latlat_c11cmn3_matl,r_res);
                % m_coast('patch',[1 .85 .7]);
                m_coast

                m_line(XY3.XY_ori(:,1),XY3.XY_ori(:,2),'Linestyle','-','color','r','linewidth',1);
                hold on
                m_line(XY3.XY_buf(:,1),XY3.XY_buf(:,2),'Linestyle','--','color','r','linewidth',1);

                if nn==use_nn(1) && ss==use_ss(end)
                    m_grid('tickdir','in','yaxislocation','left','xaxislocation','bottom');
                elseif nn==use_nn(1)
                    m_grid('tickdir','in','yaxislocation','left','xticklabels',[]);
                elseif ss==use_ss(end)
                    m_grid('tickdir','in','xaxislocation','bottom','yticklabels',[]);
                else
                    m_grid('tickdir','in','xticklabels',[],'yticklabels',[])
                end

                m_text(c11cmn3(1)+14,c11cmn3(4)+3,{['S(' num2str(ss) ')=' num2str(turn_V3_four(ss,used_sta_V))], ['N(' num2str(nn) ')\approx' num2str(round(turn_MSSA3_four_ave(ss,nn)))] }, ...
...
%             colormap(mymap('rainbow'))
        ...
        tif_name7=[fig_name,'_Spatial_STD_NSchange_SIG' num2str(Noise_SigLev) suffix_bou '.tif'];
        print(f7,'-dtiff','-r300',fullfile(fig_path_ALL3, ['R3_' tif_name7]));
    ...
    F_gap0=[.05 .125];F_marg_h0=[.33 .03];F_marg_w0=[.04 .36];
    ...
    for ss=use_ss
        ...
        for nn=use_nn
            ...
            m_line(XY3.XY_ori(:,1),XY3.XY_ori(:,2),'Linestyle','-','color','k','linewidth',1);
            hold on
            m_line(XY3.XY_buf(:,1),XY3.XY_buf(:,2),'Linestyle','--','color','r','linewidth',1);
            ...
            m_line([-50,-50,-40,-40,-50],[65,59,59,65,65],'color',[0.99,0.99,0.99],'linewidth',1.5);
            hold on

            m_text(lon_c11cmn3_matl(end)+55,lat_c11cmn3_matl(1)-9,['(' abc(ff) ')'], ...
                'color','black','fontsize',font_Size,'FontName','Times New Roman');
            ...
            switch disp_data
                case 1
                    ...
                    %                     t1=m_text(mean(lon_c11cmn3_matl),90,'EWH','FontName','Times New Roman','FontSize',font_Size,'HorizontalAlignment','center');
                case 4
                    title('EWH (noise-removal)','FontName','Times New Roman','FontSize',font_Size-4);
                    %                     t2=m_text(mean(lon_c11cmn3_matl),90,'EWH (noise-removal)','FontName','Times New Roman','FontSize',font_Size-6,'HorizontalAlignment','center');
            end
        end
...
%         colormap(mymap('rainbow'))
    ...
    for ss=use_ss
        ...
        for nn=use_nn
            ...
            m_text(c11cmn3(1)+4,c11cmn3(4),{['S(' num2str(ss) ')=' num2str(turn_V3_four(ss,used_sta_V))], ['N(' num2str(nn) ')\approx' num2str(round(turn_MSSA3_four_ave(ss,nn)))] }, ...
                'color','black','fontsize',font_Size-4,'verticalalignment','middle','horizontalalignment','left',...
            ...
            switch disp_data
                case 1
                    title('EWH','FontName','Times New Roman','FontSize',font_Size-8);
                    %                     t1=m_text(mean(lon_c11cmn3_matl),90,'EWH','FontName','Times New Roman','FontSize',font_Size,'HorizontalAlignment','center');
                case 4
                    title('EWH (noise-removal)','FontName','Times New Roman','FontSize',font_Size-4);
                    %                     t2=m_text(mean(lon_c11cmn3_matl),90,'EWH (noise-removal)','FontName','Times New Roman','FontSize',font_Size-6,'HorizontalAlignment','center');
...
%         colormap(mymap('rainbow'))
    ...
    cb = colorbar;
    cb.Location = 'South';
    cb.Position = [0.10 0.285 0.6,0.015];
    ...
    %     use_ss_nn_rms=t3_ss_nn_rms;use_sn_rms=t3_ns_rms;ybou1=[0 100];RMS_T='GRMS';
    use_ss_nn_rms=s3_ss_nn_rms;use_sn_rms=s3_ns_rms;ybou1=[0 80];RMS_T='WRMS';

    disp_sn=3; %signal rms
    use_ss_nn_square=squeeze(use_ss_nn_rms(disp_sn,:,:));


    F_gap1=[F_gap0(1) 0.06];F_marg_h1=[.06 .77];F_marg_w1=[F_marg_w0(1)+0.02 F_marg_w0e(2)];
    ...
    for i=1:length(use_nn)
        ...
        %     num_pic=(1+length(use_nn))*length(use_ss)+i;
        num_pic=i;
        ...
        for disp=use_disp
            p1(disp)=plot(S_range3,use_ss_nn_rms(disp,:,use_nn(i)),'Marker','none','LineStyle','-',...
        ...
        t=text(S_range3(1),ybou1(2)*4.5/5,['N=N(' num2str(use_nn(i)) ')'],'FontName','Times New Roman','FontSize',font_Size-3);
        t.EdgeColor='k';
        t.LineStyle='-';
        t.LineWidth=1;
    ...
    F_gap2=[.06 .03];F_marg_h2=[.34 .03];F_marg_w2=[.81 .02];
    ...
    for i=1:length(use_ss)
        ...
        pn=[];
        for disp=use_disp
            p1(disp)=plot(N_range3,reshape(use_ss_nn_rms(disp,use_ss(i),:),1,[]),'Marker','none','LineStyle','-',...
                'LineWidth',2,'color',colorr_blind(disp,:));
            hold on
            pn=[pn p1(disp)];
        end
        ...
        t=text(N_range3(1),ybou1(2)*4.5/5,['S=S(' num2str(use_ss(i)) ')'],'FontName','Times New Roman','FontSize',font_Size-3);
        t.EdgeColor='k';
        t.LineStyle='-';
        t.LineWidth=1;
    ...
    tif_name7=[fig_name,'_Spatial_Temporal_twoSTD_NSchange_SIG' num2str(Noise_SigLev) suffix_bou '.tif'];
    print(f7,'-dtiff','-r500',fullfile(fig_path_ALL3, ['R3_' tif_name7]));

    Square_Seq{SIG}=use_ss_nn_square;
end

%%
Square_Seq3=Square_Seq;
%

save([Code_Version '_' suffix_area3 '_' suffix_XY_use3  '_S' num2str(Max_S) suffix_bou '_Data.mat'],'Square_Seq3','SIG_use','use_ss','use_nn','MNC','M_rec','c11cmn','RMS_T');

%%
load([Code_Version '_' suffix_area3 '_' suffix_XY_use3  '_S' num2str(Max_S) suffix_bou '_Data.mat'])
...
for jj=1:length(SIG_use)
    ...
    use_ss_nn_square_SIG=Square_Seq3{jj};
    % F3 (new)
...
%
fprintf('Then we determine the optimal N (in variable ''SIG_nn'') and S (in variable ''SIG_ss'') corresponding to different p.');

save([Code_Version '_' suffix_area3 '_' suffix_XY_use3 '_S' num2str(Max_S)  suffix_bou '_Data.mat'],'SIG_ss','SIG_nn','-append');
...
Noise_SigLev=0.05;
R95=load(fullfile(getenv('IFILES'),['Results_' parrent_path3],['Main_' Attach_ALL3 '_TP' num2str(MNC) ...
...
R90=load(fullfile(getenv('IFILES'),['Results_' parrent_path3],['Main_' Attach_ALL3 '_TP' num2str(MNC) ...
...
Noise_SigLev=0.3;
% load(fullfile(getenv('IFILES'),['Results_' parrent_path],['Main_' Attach_ALL '_S' num2str(lamda_range(ss)*100) '_N' num2str(lamda_range(nn)*100) '_compare.mat']))
R70=load(fullfile(getenv('IFILES'),['Results_' parrent_path3],['Main_' Attach_ALL3 '_TP' num2str(MNC) ...
...
for ins=1:3
    ...
    fill_MASS=A5_Centers_Compare(ins).fill_MASS(fit_data_length);
    fill_MASS_resid=A5_Centers_Compare(ins).fill_MASSres(fit_data_length);

    A5_Centers_Compare(ins).fill_MASS_Std=std(fill_MASS);

    % kk=kk-mean(kk);

    [signal_kk,resid_kk,~,~,...
        Amplitude,Phase]=periodic_analysis_m(fill_MASS',...
    ...
    A5_Centers_Compare(ins).fill_MASS_Amp=Amplitude;
    A5_Centers_Compare(ins).fill_MASS_Pha=Phase/pi*180;

    A5_Centers_Compare(ins).fill_MASS_signal_Std=std(signal_kk);
    A5_Centers_Compare(ins).fill_MASS_resid_Std=std(resid_kk);

    [fit_MASS,delta_MASS,totalparams_MASS,paramerrors_MASS] = timeseriesfit([fill_dates' fill_MASS'], ...
        var(resid_kk),1,1);
    A5_Centers_Compare(ins).fill_MASS_slope=totalparams_MASS(2)*365.25;
    A5_Centers_Compare(ins).fill_MASS_fit = [fill_dates' fit_MASS delta_MASS];
    A5_Centers_Compare(ins).fill_MASS_paramerrors = paramerrors_MASS(2)*365.25; % this (365.25) is special for 'timeseriesfit' results.

    % seasonal and deseasonal (trend is removed)
    fill_MASS_detrend=fill_MASS-fit_MASS';
    fill_MASS_deseason=fill_MASS_detrend;
    fill_MASS_season=zeros(1,12);fill_MASS_season_notdetrend=zeros(1,12);
    for j=1:12
        fill_MASS_season_notdetrend(j)=mean(fill_MASS(j:12:end));
        fill_MASS_season(j)=mean(fill_MASS_detrend(j:12:end));

        fill_MASS_deseason(j:12:end)=fill_MASS_deseason(j:12:end)-fill_MASS_season(j);
    end
    A5_Centers_Compare(ins).fill_MASS_season=fill_MASS_season;
    A5_Centers_Compare(ins).fill_MASS_deseason=fill_MASS_deseason;
end
...
for ins=1:3
    ...
    fill_dates=A5_Centers_Compare(ins).fill_dates(fit_data_length);
    fill_EWH=A5_Centers_Compare(ins).fill_Grid_EWH(fit_data_length,:,:);
    %     fill_EWH_resid=A5_Centers_Compare(ins).fill_Grid_EWHres;

    [t,m,n]=size(fill_EWH);

    A5_Centers_Compare(ins).fill_Grid_NaN=nan(m,n);
    A5_Centers_Compare(ins).fill_EWH_Std=nan(m,n);
    A5_Centers_Compare(ins).fill_Grid_EWH_Amp=nan(numel(fitwhat)-1,m,n);
    A5_Centers_Compare(ins).fill_Grid_EWH_Pha=nan(numel(fitwhat)-1,m,n);
    A5_Centers_Compare(ins).fill_Grid_EWHsig=nan(t,m,n);
    A5_Centers_Compare(ins).fill_Grid_EWHres=nan(t,m,n);
    A5_Centers_Compare(ins).fill_Grid_EWH_signal_Std=nan(m,n);
    A5_Centers_Compare(ins).fill_Grid_EWH_resid_Std=nan(m,n);
    A5_Centers_Compare(ins).fill_Grid_EWH_slope=nan(m,n);
    A5_Centers_Compare(ins).fill_Grid_EWH_fit ={};
    A5_Centers_Compare(ins).fill_Grid_EWH_paramerrors = nan(m,n) ; % this (365.25) is special for 'timeseriesfit' results.

    A5_Centers_Compare(ins).fill_Grid_EWH_season=nan(12,m,n);
    A5_Centers_Compare(ins).fill_Grid_EWH_deseason=nan(t,m,n);
    A5_Centers_Compare(ins).fill_Grid_EWH_4seasons=nan(4,m,n);
    A5_Centers_Compare(ins).fill_Grid_EWH_detrend=nan(t,m,n);

    A5_Centers_Compare(ins).fill_Grid_EWHsig_season=nan(12,m,n);
    A5_Centers_Compare(ins).fill_Grid_EWHsig_deseason=nan(t,m,n);
    A5_Centers_Compare(ins).fill_Grid_EWHsig_4seasons=nan(4,m,n);

    A5_Centers_Compare(ins).fill_Grid_EWHres_season=nan(12,m,n);
    A5_Centers_Compare(ins).fill_Grid_EWHres_deseason=nan(t,m,n);
    A5_Centers_Compare(ins).fill_Grid_EWHres_4seasons=nan(4,m,n);
    ...
    for mm=1:m
        ...
        for nn=1:n
            ...
            fill_one_EWH=fill_EWH(:,mm,nn)';

            if isnan(fill_one_EWH)
                A5_Centers_Compare(ins).fill_Grid_NaN(mm,nn)=sum(isnan(fill_one_EWH));
                continue
            end

            A5_Centers_Compare(ins).fill_EWH_Std(mm,nn)=std(fill_one_EWH);

            % kk=kk-mean(kk);

            [signal_kk,resid_kk,~,~,...
                Amplitude,Phase]=periodic_analysis_m(fill_one_EWH',...
                fill_dates,fitwhat); % sin(wt-fi)
            Phase=pi/2-Phase; % be consistent with previous studies for cos(wt+fi)
            Phase(Phase<0)=Phase(Phase<0)+2*pi; % -180~-180 to 0~360
            A5_Centers_Compare(ins).fill_Grid_EWH_Amp(:,mm,nn)=Amplitude;
            A5_Centers_Compare(ins).fill_Grid_EWH_Pha(:,mm,nn)=Phase;

            A5_Centers_Compare(ins).fill_Grid_EWHsig(:,mm,nn)=signal_kk;
            A5_Centers_Compare(ins).fill_Grid_EWHres(:,mm,nn)=resid_kk;

            A5_Centers_Compare(ins).fill_Grid_EWH_signal_Std(mm,nn)=std(signal_kk);
            A5_Centers_Compare(ins).fill_Grid_EWH_resid_Std(mm,nn)=std(resid_kk);

            [fit_EWH,delta_EWH,totalparams_EWH,paramerrors_EWH] = timeseriesfit([fill_dates' fill_one_EWH'], ...
                var(resid_kk),1,1);
            A5_Centers_Compare(ins).fill_Grid_EWH_slope(mm,nn)=totalparams_EWH(2)*365.25;
            A5_Centers_Compare(ins).fill_Grid_EWH_fit{mm,nn} = [fill_dates' fit_EWH delta_EWH];
            A5_Centers_Compare(ins).fill_Grid_EWH_paramerrors(mm,nn) = paramerrors_EWH(2)*365.25; % this (365.25) is special for 'timeseriesfit' results.

            % seasonal
            fill_EWH_detrend=fill_one_EWH-fit_EWH';
            A5_Centers_Compare(ins).fill_Grid_EWH_detrend(:,mm,nn)=fill_EWH_detrend;

            fill_EWH_deseason=fill_EWH_detrend;fill_EWH_season=zeros(1,12);
            fill_EWH_deseason_season=zeros(1,12);
            for j=1:12
                fill_EWH_season(j)=mean(fill_EWH_detrend(j:12:end));

                fill_EWH_deseason(j:12:end)=fill_EWH_deseason(j:12:end)-fill_EWH_season(j);

                fill_EWH_deseason_season(j)=mean(fill_EWH_deseason(j:12:end));
            end
            A5_Centers_Compare(ins).fill_Grid_EWH_season(:,mm,nn)=fill_EWH_season;
            A5_Centers_Compare(ins).fill_Grid_EWH_deseason(:,mm,nn)=fill_EWH_deseason;

            A5_Centers_Compare(ins).fill_Grid_EWH_4seasons(1,mm,nn)=mean(fill_EWH_season(4:6));
            A5_Centers_Compare(ins).fill_Grid_EWH_4seasons(2,mm,nn)=mean(fill_EWH_season(7:9));
            A5_Centers_Compare(ins).fill_Grid_EWH_4seasons(3,mm,nn)=mean(fill_EWH_season(10:12));
            A5_Centers_Compare(ins).fill_Grid_EWH_4seasons(4,mm,nn)=mean(fill_EWH_season(1:3));

            % seasonal signal
            fill_EWHsig_deseason=signal_kk;fill_EWHsig_season=zeros(1,12);
            fill_EWHres_deseason=resid_kk;fill_EWHres_season=zeros(1,12);
            for j=1:12
                fill_EWHsig_season(j)=mean(signal_kk(j:12:end));
                fill_EWHres_season(j)=mean(resid_kk(j:12:end));

                fill_EWHsig_deseason(j:12:end)=fill_EWHsig_deseason(j:12:end)-fill_EWHsig_season(j);
                fill_EWHres_deseason(j:12:end)=fill_EWHres_deseason(j:12:end)-fill_EWHres_season(j);
            end
            A5_Centers_Compare(ins).fill_Grid_EWHsig_season(:,mm,nn)=fill_EWHsig_season;
            A5_Centers_Compare(ins).fill_Grid_EWHsig_deseason(:,mm,nn)=fill_EWHsig_deseason;
            A5_Centers_Compare(ins).fill_Grid_EWHres_season(:,mm,nn)=fill_EWHres_season;
            A5_Centers_Compare(ins).fill_Grid_EWHres_deseason(:,mm,nn)=fill_EWHres_deseason;

            A5_Centers_Compare(ins).fill_Grid_EWHsig_4seasons(1,mm,nn)=mean(fill_EWHsig_season(4:6));
            A5_Centers_Compare(ins).fill_Grid_EWHsig_4seasons(2,mm,nn)=mean(fill_EWHsig_season(7:9));
            A5_Centers_Compare(ins).fill_Grid_EWHsig_4seasons(3,mm,nn)=mean(fill_EWHsig_season(10:12));
            A5_Centers_Compare(ins).fill_Grid_EWHsig_4seasons(4,mm,nn)=mean(fill_EWHsig_season(1:3));

            A5_Centers_Compare(ins).fill_Grid_EWHres_4seasons(1,mm,nn)=mean(fill_EWHres_season(4:6));
            A5_Centers_Compare(ins).fill_Grid_EWHres_4seasons(2,mm,nn)=mean(fill_EWHres_season(7:9));
            A5_Centers_Compare(ins).fill_Grid_EWHres_4seasons(3,mm,nn)=mean(fill_EWHres_season(10:12));
            A5_Centers_Compare(ins).fill_Grid_EWHres_4seasons(4,mm,nn)=mean(fill_EWHres_season(1:3));
...
ax_name=["STPC(5%)","STPC(10%)","STPC(30%)"];
...
for ins=comp_data
    ...
    p1(ff)=plot(tt_use,A5_Centers_Compare(ins).fill_MASS(fit_data_length),'linewidth',2,'color',colorr_use(ins,:));
    hold on
    p2(ff)=plot(tt_use,A5_Centers_Compare(ins).fill_MASS_fit(:,2),'color',colorr_use(ins,:),'linewidth',1);
    hold on
    %     p3(ff)=plot(tt_fil,(A5_Centers_Compare(ins).fill_EWH_fit(:,2)+A5_Centers_Compare(ins).fill_EWH_fit(:,3)),'m','linestyle','--','linewidth',0.7);
    %     hold on
    %     p4(ff)=plot(tt_fil,(A5_Centers_Compare(ins).fill_EWH_fit(:,2)-A5_Centers_Compare(ins).fill_EWH_fit(:,3)),'m','linestyle','--','linewidth',0.7);
    %     hold on
    %
    pn=[pn p1(ff)];
    lg{ff}=[char(ax_name(ins)) ': ' num2str(A5_Centers_Compare(ins).fill_MASS_slope,'%.3f') ' \pm ' ...
        num2str(A5_Centers_Compare(ins).fill_MASS_paramerrors,'%.3f') ' Gt/yr'] ;
...
tif_name1=['Temporal_EWH_compare_other_SIG.tif'];
print(f1,'-dtiff','-r300',fullfile(fig_path_ALL3,tif_name1));
