...
function varargout = MSSA_gap_fitting_correct(A5_CJGI_Sort,M,N,fig_note)
...
% first M-SSA
for i=1:intit_num
    MSLA1_CJGI(:,i)=A5_CJGI_Sort(i).MSSA_TS';
end
...
for i=1:intit_num
    ...
    tt_use=floor(A5_CJGI_Sort(i).use_months/12)+data_year_beg+mod(A5_CJGI_Sort(i).use_months,12)/12-1/24;
    tt_fil=floor(fill_months/12)+data_year_beg+mod(fill_months,12)/12-1/24;
    tt_lea=floor(A5_CJGI_Sort(i).missing_months/12)+data_year_beg+mod(A5_CJGI_Sort(i).missing_months,12)/12-1/24;

    missing_months=A5_CJGI_Sort(i).missing_months;
    MSSA_TS=A5_CJGI_Sort(i).MSSA_TS;
